...
public class GUIBlockSelect extends GUI {
	
	ArrayList<GridBlock> avail = new ArrayList<GridBlock>();
	PanelMapCreate p;
	int buttonSize, page = 0, pageSize = 8*4;
	Rectangle back, forward;
	
	public GUIBlockSelect(PanelMapCreate p) {
		...
		for(int i = 1; i < GridBlock.all.length; ++i){
			if(i<GridBlock.all.length && GridBlock.all[i]!=null)
			avail.add(GridBlock.all[i]);
		}
	...
	public void click(boolean b){
		...
		for(int j = 0; j < avail.size(); ++j){
			int i = j+1;
			Rectangle rect = new Rectangle(x+i*(buttonSize)-(i/(w/buttonSize))*(buttonSize)*(w/buttonSize), y+(i/(w/buttonSize))*(buttonSize)-(page*buttonSize*4), buttonSize, buttonSize);
			if(rect.contains(Display.currentScreen.mousePoint)){
				confirm(avail.get(j).getID());
			}
		}
	}
	...
	public void draw(Graphics g, int x, int y) {
		...
		for(int j = page*pageSize; j < (page+1)*pageSize; ++j){
			...
			if(avail.size() > j){
				...
				g.drawImage(avail.get(j).getImage(), rect.x+10,  rect.y+10, buttonSize-20, buttonSize-20, null);

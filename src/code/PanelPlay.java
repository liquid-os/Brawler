...
public class PanelPlay extends GridPanel{
	
	String clientUser = Properties.username, clientUser1 = Properties.username+"1";
	...
	public PanelPlay(String map){
		super(new Grid(Properties.width, null));
		Util.music = (Properties.selHero==Hero.snoopdog?new ClipShell("snooploop.wav"):Util.getMusic());
		Util.music.loop(-1);
	...
	public PanelPlay(String map, boolean str){
		super(new Grid(Properties.width, null));
		Util.music = (Properties.selHero==Hero.snoopdog?new ClipShell("snooploop.wav"):Util.getMusic());
		Util.music.loop(-1);
		this.background = Color.WHITE;
	...
	public void onDeath(int i, boolean isClient){
		players.get(i).onDeath();
			...
			if(players.get(i).respawns<=0){
				...
				Date date = new Date();
				String kn = Properties.selHero.classname+" "+dateFormat.format(date);
				Robot robot;
				try {
					new File(Bank.path+"killcams/").mkdirs();
					robot = new Robot();
			        BufferedImage bi = robot.createScreenCapture(new Rectangle(players.get(i).posX+players.get(i).width/2-250, players.get(i).posY+players.get(i).height/2-250, 500, 500));
					ImageIO.write(bi,"png",new File(Bank.path+"killcams/"+kn+".png"));
					Util.persistentGuis.add(new GUINotify(bi, "Kill Saved!", kn+".png", 0, 0));
	...
	public void keyPressed(KeyEvent e){
		for(int i = 0; i < players.size(); ++i){
			...
			if(players.get(i).getUsername().equals(clientUser1))players.get(i).keyPressed(e);
		}
	}
	...
	public void keyReleased(KeyEvent e){
		for(int i = 0; i < players.size(); ++i){
			...
			if(players.get(i).getUsername().equals(clientUser1))players.get(i).keyReleased(e);
		}
